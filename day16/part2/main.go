...
func (program *Program) From(input string) {
	bytes := []byte(input)
	for i := 0; i < program.length; i++ {
		b := bytes[i] - 'a'
		program.positions[b] = i
		program.bytes[i] = b
	}
	program.offset = 0
}
...
func main() {
	...
	var result string
	// Don't need to run exactly 1 billion times
	// since the result converges to a single value
	for i := 0; i < 1000*1000; i++ {
		if val, ok := cache[result]; ok {
			program.From(val)
